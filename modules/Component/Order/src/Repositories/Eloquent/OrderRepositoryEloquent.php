...
class OrderRepositoryEloquent extends BaseRepository implements OrderRepository
{
	...
	{
		return $this->with(['items', 'shippings', 'invoices'])->findByField('order_no', $no)->first();
	}
	...
	public function getOrderCountByUserAndStatus($user_id, $status)
	{
		...
		return $this->model->where('user_id', $user_id)->where('status', $status)->where('type', '<>', Order::TYPE_POINT)->count();
	}
}
