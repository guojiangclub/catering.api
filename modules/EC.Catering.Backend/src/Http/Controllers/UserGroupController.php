...
class UserGroupController extends Controller
{
	...
	public function grouplist()
	{
		...
		return LaravelAdmin::content(function (Content $content) use ($group) {
			...
			$content->body(view('catering-backend::user_group.grouplist', compact('group')));
		});
	}
	...
	public function groupcreate(Request $request)
	{
		...
		$rights = UserRights::where('status', 1)->get();
		...
		return LaravelAdmin::content(function (Content $content) use ($group_edit, $action, $flag, $title, $rights) {
			$header = '新增会员等级';
			if (request('id')) {
				$header = '编辑会员等级';
			}
			...
			$content->body(view('catering-backend::user_group.groupstore', compact('group_edit', 'action', 'flag', 'title', 'rights')));
		});
	}
	...
	public function groupstore(Request $request)
	{
		if (request('grade') or '0' == request('grade')) {
			...
			if ($group_list && $group_list->id != request('id')) {
