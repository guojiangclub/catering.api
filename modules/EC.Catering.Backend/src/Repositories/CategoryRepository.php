...
class CategoryRepository extends BaseRepository
{
	 ...
	 * @return mixed
	 */
	public function getSortCategory($groupID)
	{
		if ($groupID) {
			$category = $this->scopeQuery(function ($query) use ($groupID) {
				return $query->where('group_id', $groupID)->where('level', '<=', 2)->orderBy('sort', 'asc');
			})->all(['id', 'name', 'parent_id', 'sort', 'level', 'path']);
		} else {
	 ...
	 * @return array
	 */
	public function getLevelCategory($groupID, $pid = 0, $html = ' ', $dep = '')
	{
	...
	public function delCategory($id)
	{
		...
		if (count($this->findWhere(['parent_id' => $id])) > 0) {
			return false;
		}
