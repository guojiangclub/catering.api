...
//    [1] => {"id":"2","type":"1","value":"Ocean Blue","name":"颜色"}
    ...
    public function getExcelGoods($limit = 50)
    {
        ...
        $specs    = DB::table(config('ibrand.app.database.prefix', 'ibrand_') . 'goods_specs_value')->join(config('ibrand.app.database.prefix', 'ibrand_') . 'goods_spec', 'el_goods_spec.id', '=', config('ibrand.app.database.prefix', 'ibrand_') . 'goods_specs_value.spec_id')
            ->select(config('ibrand.app.database.prefix', 'ibrand_') . 'goods_specs_value.id as id', config('ibrand.app.database.prefix', 'ibrand_') . 'goods_specs_value.spec_id as spec_id', config('ibrand.app.database.prefix', 'ibrand_') . 'goods_spec.name as spec', config('ibrand.app.database.prefix', 'ibrand_') . 'goods_specs_value.name as value')
            ->get();
            ...
            if (is_array($specIds)) {
                foreach ($specIds as $id) {
                    ...
                    if ($spec) {
                        ...
                        if ($spec->spec_id == 2) {
                            $alias                   = DB::table(config('ibrand.app.database.prefix', 'ibrand_') . 'goods_spec_relation')->where('spec_value_id', $spec->id)->where('goods_id', $product['id'])->first();
