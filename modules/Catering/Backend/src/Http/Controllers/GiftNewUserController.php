...
class GiftNewUserController extends Controller
{
    ...
    public function index()
    {
        ...
        $lists = $this->giftActivityRepository->getGiftActivityPaginated();
        ...
        return Admin::content(function (Content $content) use ($lists, $coupons) {
            ...
            $content->breadcrumb(
                ['text' => '新人进店礼', 'no-pjax' => 1, 'left-menu-active' => '新人进店礼']
            );

            $content->body(view('backend-shitang::gift.index', compact('lists', 'coupons')));
        });
    }
    ...
    public function create()
    {
        ...
        return Admin::content(function (Content $content) use ($coupons) {
            ...
            $content->breadcrumb(
                ['text' => '添加活动', 'no-pjax' => 1, 'left-menu-active' => '新人进店礼']
            );

            $content->body(view('backend-shitang::gift.create', compact('coupons')));
        });
    }
    ...
    public function edit($id)
    {
        ...
        return Admin::content(function (Content $content) use ($activity, $coupons, $selected) {
            ...
            $content->breadcrumb(
                ['text' => '编辑活动', 'no-pjax' => 1, 'left-menu-active' => '新人进店礼']
            );

            $content->body(view('backend-shitang::gift.edit', compact('activity', 'coupons', 'selected')));
        });
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($input['status']) {
            ...
            if ($activity) {
                return $this->ajaxJson(false, [], 500, '当前集市已有正在进行的活动');
            }
        }
    ...
    public function toggleStatus(Request $request)
    {
        ...
        $where = ['status' => 1,'activity_type'=>'gift_new_user', ['starts_at', '<=', Carbon::now()], ['ends_at', '>', Carbon::now()], ['id', '!=', $id]];
