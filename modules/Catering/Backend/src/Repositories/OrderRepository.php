...
class OrderRepository extends BaseRepository
{
    ...
    {
        return $this->with('user')->findByField('order_no', $no)->first();
    }
...
//            $query =$query->where(['status', '>', '0']);
    ...
    public function getExportOrdersData($where, $limit = 50, $time = [], $more = [], $pay_time = [])
    {
        ...
        $data = $data->with('payments', 'adjustments', 'user')->orderBy('created_at', 'desc');
    ...
    public function formatToExcelData($orders)
    {
        ...
        if (!is_null($orders) && $orders->count() > 0) {
            ...
            foreach ($orders as $order) {
                $data[$i][] = $order->order_no;
                $data[$i][] = $order->payment_text;
                $data[$i][] = $order->order_user_name;
                $data[$i][] = $order->StatusText;
                $data[$i][] = $order->items_total_yuan;
                $data[$i][] = abs($order->adjustments_total_yuan);
                $data[$i][] = $order->used_balance_amount;
                $data[$i][] = $order->paid_amount;
                $data[$i][] = $order->created_at;
                $data[$i][] = $order->pay_time;
                $data[$i][] = $order->note;
                $i++;
            }
        }
        ...
        if ($users->total() > 0) {
            ...
            if (isset($where['spu'])) {
                ...
                $products          = DB::table(config('ibrand.app.database.prefix', 'ibrand_') . 'goods')
                    ...
                    ->join(config('ibrand.app.database.prefix', 'ibrand_') . 'goods_product', config('ibrand.app.database.prefix', 'ibrand_') . 'goods.id', '=', config('ibrand.app.database.prefix', 'ibrand_') . 'goods_product.goods_id')
        ...
        if (isset($more['supplier']) AND $more['supplier']) {
            if ($ids) {
                $users = DB::table($prefix . 'order')
                    ->join($prefix . 'order_item', 'el_order.id', '=', 'el_order_item.order_id')
