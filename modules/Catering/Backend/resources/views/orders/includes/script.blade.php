...
<script>
    ...
    $(document).on('click.modal.data-api', '[data-toggle="modal-filter"]', function (e) {
        ...
        var url = '{{route('admin.shitang.orders.getExportData')}}';
        var type = $(this).data('type');
    ...
    /*合并订单批量发货*/
    $(document).on('click.modal.data-api', '[data-toggle="multiple_modal"]', function (e) {
        var $this = $(this),
                href = $this.attr('href'),
                modalUrl = $(this).data('url');
        ...
        var inputIds = $('input[name="order_id[' + uid + '][]"]');
        var orderIds = '';
        $(inputIds).each(function (index, item) {
            orderIds += 'ids[]=' + $(item).val() + '&';
        });

        modalUrl = modalUrl + '&' + orderIds;
